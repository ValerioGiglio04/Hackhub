...
public class InvitiStaffHandler {
   ...
   */
  public InvitoStaff invitaStaff(Long hackathonId, Long utenteInvitatoId, Long mittenteId) {
    if (hackathonRepository == null || utenteRepository == null || staffHackatonRepository == null) {
      throw new IllegalStateException("HackathonRepository, UtenteRepository e StaffHackatonRepository richiesti per invitaStaff");
    }
    Hackathon hackathon = hackathonRepository.findById(hackathonId)
        .orElseThrow(() -> new EntityNotFoundException("Hackathon", hackathonId));
    Utente staff = utenteRepository.findById(utenteInvitatoId)
    ...
    if (mittente.getRuolo() != Utente.RuoloStaff.ORGANIZZATORE) {
      throw new BusinessLogicException("Solo un organizzatore può invitare staff a un hackathon.");
    }
    ...
    List<StaffHackaton> staffAssegnati = staffHackatonRepository.findByHackathonId(hackathonId);
    boolean giaAssegnato = staffAssegnati.stream()
        .anyMatch(sh -> sh.getUtente() != null && utenteInvitatoId.equals(sh.getUtente().getId()));
    if (giaAssegnato) {
      throw new BusinessLogicException("L'utente è già assegnato a questo hackathon.");
    }
    Optional<InvitoStaff> invitoEsistente = invitoStaffRepository
        .findByHackathonIdAndUtenteInvitatoIdAndStato(hackathonId, utenteInvitatoId, InvitoStaff.StatoInvito.PENDING);
    if (invitoEsistente.isPresent()) {
      throw new BusinessLogicException("Esiste già un invito pendente per questo utente a questo hackathon.");
    }
    if (hackathon.getStato() == Hackathon.StatoHackathon.CONCLUSO) {
      throw new BusinessLogicException("L'hackathon è già concluso. Non è possibile invitare staff.");
    }
    long numOrg = staffAssegnati.stream().filter(sh -> sh.getUtente() != null && sh.getUtente().getRuolo() == Utente.RuoloStaff.ORGANIZZATORE).count();
    long numMentori = staffAssegnati.stream().filter(sh -> sh.getUtente() != null && sh.getUtente().getRuolo() == Utente.RuoloStaff.MENTORE).count();
    long numGiudici = staffAssegnati.stream().filter(sh -> sh.getUtente() != null && sh.getUtente().getRuolo() == Utente.RuoloStaff.GIUDICE).count();
    if (staff.getRuolo() == Utente.RuoloStaff.ORGANIZZATORE && numOrg >= MAX_ORGANIZZATORI) {
      throw new BusinessLogicException("L'hackathon ha già raggiunto il numero massimo di " + MAX_ORGANIZZATORI + " organizzatori.");
    }
    if (staff.getRuolo() == Utente.RuoloStaff.MENTORE && numMentori >= MAX_MENTORI) {
      throw new BusinessLogicException("L'hackathon ha già raggiunto il numero massimo di " + MAX_MENTORI + " mentori.");
    }
    if (staff.getRuolo() == Utente.RuoloStaff.GIUDICE && numGiudici >= MAX_GIUDICI) {
      throw new BusinessLogicException("L'hackathon ha già raggiunto il numero massimo di " + MAX_GIUDICI + " giudici.");
    }

...
*Header Auth: Token Utente 6 (capoteam1)*
```json
{
  "nome": "Nuovo Team",
  "capoId": 6,
...
```
*Nota: Se l’utente 6 è già capo di un team (CodeCrushers), usa quel team per i passi successivi. Altrimenti copia l’`id` del team creato (es. `TEAM_ID`).*

**2. Invito a team (da Capo a Utente 7)**  
`POST http://localhost:8080/api/inviti/invia-invito`  
*Header Auth: Token Capo Team*
```json
{
  "teamId": 1,
  "utenteInvitatoId": 7
...
```
*Nota: Copia l’`id` dell’invito dalla risposta (es. `INVITO_ID`).*
...
*Header Auth: Token Organizzatore*

*Nota: Assicurati che le date siano coerenti (iscrizioni, inizio/fine, scadenza sottomissioni) per permettere il flusso di test.*
```json
{
...
```
*Nota: Copia l’`id` dell’hackathon (es. `HACKATHON_ID`).*
...
```
*(Ripeti per altri mentori/giudici se necessario; i mentori/giudici accettano da `GET /api/staff-inviti/ricevuti` e `POST /api/staff-inviti/{invitoId}/gestisci-invito`.)*
...
*Header Auth: Token Capo Team o Membro*

*Nota: L’hackathon deve essere in fase IN_CORSO (tra dataInizio e dataFine). Lo scheduler aggiorna gli stati; in sviluppo puoi usare date che ricadono “in corso” nel momento del test.*
```json
{
  "teamId": 1,
  "hackathonId": 1,
...
```
*Nota: Copia l’`id` della sottomissione (es. `SOTTOMISSIONE_ID`).*

**2. Valutazione (Giudice)**  
`POST http://localhost:8080/api/valutazioni/crea`  
*Header Auth: Token Giudice*

*Nota: Il giudice deve essere assegnato all’hackathon della sottomissione. L’hackathon deve essere in fase di valutazione.*
```json
{
  "sottomissioneId": 1,
  "hackathonId": 1,
...
```
*(Il campo `giudiceId` viene impostato automaticamente dal backend.)*
